...
class ImageViewController: NSViewController {
    ...
    /// The view that will display the image.
    private let imageView: NSImageView = {
        let iv = NSImageView()
        iv.translatesAutoresizingMaskIntoConstraints = false
        iv.imageScaling = .scaleProportionallyUpOrDown // Ensures the whole image is visible
        iv.animates = true
        return iv
    }()
    
    /// A label to show if the image cannot be loaded.
    private let errorLabel: NSTextField = {
        let label = NSTextField(labelWithString: "Error: Could not load image.")
    ...
    /// Initializes the view controller with a specific image URL.
    init(imageURL: URL) {
        self.imageURL = imageURL
        super.init(nibName: nil, bundle: nil)
    }

    /// Required initializer for storyboards/xibs, which we aren't using.
    required init?(coder: NSCoder) {
        fatalError("init(coder:) has not been implemented. Use init(imageURL:) instead.")
    }

    // MARK: - View Lifecycle

    override func loadView() {
        // The main view is just a generic container.
        self.view = NSView()
    }
    ...
    private func setupUI() {
        // Add the image view and error label to the main view
        view.addSubview(imageView)
        view.addSubview(errorLabel)
        
        // Set up constraints to make the image view fill the entire view,
        // and the error label to be centered.
        NSLayoutConstraint.activate([
            // Pin image view to all four edges of the parent view
    ...
    private func loadImage() {
        // Create an NSImage from the URL. This can fail if the path is wrong
        // or the file isn't a valid image.
        if let image = NSImage(contentsOf: imageURL) {
            imageView.image = image
            errorLabel.isHidden = true
        } else {
            // If loading fails, hide the image view and show the error label.
